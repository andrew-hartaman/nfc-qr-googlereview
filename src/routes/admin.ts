...
adminRouter.patch('/api/cards/:identifier', async (c) => {
  ...
  try {
    ...
    if (body.is_active !== undefined) {
      // If target_url is null/empty, card cannot be active. Otherwise respect user's explicit toggle.
      const hasUrl = updatePayload.target_url !== undefined
        ? updatePayload.target_url !== null
        : existingCard.target_url !== null;
      updatePayload.is_active = hasUrl ? body.is_active : false;
    } else if (body.target_url !== undefined) {
      // Auto-toggle only if is_active was not explicitly provided in the request
 ...
 * Tahap A: Bulk generates empty cards with random short_codes and autogenerated sequential labels
 */
adminRouter.post('/api/cards/generate-batch', async (c) => {
  ...
  const freeString = body?.label ? body.label.trim() : null;

  try {
    const supabase = getSupabaseClient(c.env);

    // Find the highest existing K- sequence number in cards table
    ...
    const currentMax = extractMaxSequenceFromLabels(existingCards?.map((c) => c.label) || []);
    const generated = [];
    ...
    for (let i = 0; i < count; i++) {
      ...
      while (!isUnique && attempts < 3) {
        ...
        const { data } = await supabase.from('cards').select('id').ilike('short_code', shortCode).maybeSingle();
      ...
      const sequence = currentMax + i + 1;
      const cardLabel = formatCardLabel(sequence, freeString);
